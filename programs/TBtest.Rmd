...
It does not seem like steeper than exponential selection will yield and scenarios where higher order moments of the breeding value distribution are important for the response to selection. The reason for this is that as selection becomes strong enough that second order terms of the selectin coefficient are important it is also strong enough that the weak selection approximation of Turelli and Barton breaks down. As a final test of the potential importance of deviations from normality on the response to selection I'll consider a cubic selection function. We'll play the same game as before and investigate what parameter space we get a good description of the response to selection.
```{r}
...
```{r}
cube.seltest <- select.pop(pop$ae, fit.func=cub.sel,b0=50, b3=1)
...
We can see that the change in the mean is rather small, but also that there is a shift in the phenotype distribution that is distinc from the two previous modes of selection we considered. Let's now write down equations that should tell us what the response to selection should be.
```{r}
wbar <- function(b0, b3, zbar, Vg, M3g){
  return(b0 + b3 * zbar^3 + 3*Vg*b3*zbar + M3g*b3)
}
L1.3 <- function(b0, b3, zbar, Vg, M3g){
  return(
    (3*b3*zbar^2 + Vg * 3*b3) / wbar(b0, b3, zbar, Vg, M3g)
  )
}
L2.3 <- function(b0, b3, zbar, Vg, M3g){
  return(
    3*b3*zbar / wbar(b0, b3, zbar, Vg, M3g)
  )
}
...
```{r}
e.change <- e.dz.3(b0=200, b3=0.05, zbar=mean(pop$ie), Vg=var(pop$ie), M3g=moment(pop$ie, order=3, central=TRUE),
                   M4g=moment(pop$ie, order=4, central=TRUE))
cube.tests <- replicate(500, select.pop(pop$ae, fit.func=cub.sel,b0=50, b3=.05))
