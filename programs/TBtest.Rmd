...
```{r}
library(ggplot2)
make.pop <- function(NN=1000, ll=10, mut.dist=rnorm, ...){
...
test.pop.2 <- make.pop(NN=1000, ll=100, mean=0, sd=1)
...
select.pop <- function(ind.eff, fit.func=exp.sel, ...){
    fitness.values <- apply(ind.eff, MARGIN=1, FUN=function(x.set){
        return(fit.func(x=sum(x.set), ...))})
...
$$
L_1 = \frac{A + \frac{3V_g}{2}AB + \frac{M_{3,g}}{2}B^2}{1 + \frac{V_g}{2}\left( A + B \right) +
  \frac{M_{3,g}}{2}AB + \frac{M_{4.g}}{8}B^2},
$$
$$
L_2 = \frac{B+A^2}{2\left( 1 + \frac{V_g}{2}\left( A + B \right) +
  \frac{M_{3,g}}{2}AB + \frac{M_{4.g}}{8}B^2\right)},
$$
$$
L_3 = \frac{AB}{2\left( 1 + \frac{V_g}{2}\left( A + B \right) +
  \frac{M_{3,g}}{2}AB + \frac{M_{4.g}}{8}B^2\right)},
$$
and 
$$
L_4 = \frac{B^2}{8\left( 1 + \frac{V_g}{2}\left( A + B \right) +
  \frac{M_{3,g}}{2}AB + \frac{M_{4.g}}{8}B^2\right)}.
$$
Where $A = \frac{2\bar{Y}\epsilon + s}{1 - 2\epsilon V_e}$ and $B =\frac{2\epsilon}{1 - 2\epsilon V_e}$. These are approximations that ignore terms of order three or greater in $A$ and $B$. It is also worth noting that the selection differentials now include moments of the population variation in breeding values. Let us first define functions to calculate these.
```{r}
...
L1 <- function(Ybar, ee, ss, Ve, Vg, M3g, M4g){
    ...
    return( (A + 0.5 * Vg * A^3 + 3 * Vg * A * B + 2 * M3g * B^2) / 
...
```
Now let's select a population using an intermediate level of steeper than exponential selection with $s=0.1$ and $\epsilon=0.01$. 
```{r}
s.test <- 0.005
e.test <- 0.008
ee=e.test; ss=s.test; Ve=0
Vg=var(test.pop.2$ie)
M4g=moment(test.pop.2$ie, order=4, central=TRUE)
M3g=moment(test.pop.2$ie, order=3, central=TRUE)
A <- AA(Ybar, ee, ss, Ve); B <- BB(ee, Ve)
...
st.ts <- data.frame(cbind(before.sel=as.numeric(test.pop.2$ie),
                       after.sel=rowSums(test.select.st)))
...
e.dz <- s.test * var(test.pop.2$ie) + s.test^2 * moment(test.pop.2$ie, 
                                                   order=3, central=TRUE)
Ybar=mean(test.pop.2$ie)

## for testing purposes


e.dz.st <- e.steep.dz(Ybar=mean(test.pop.2$ie), 
                      ee=e.test, ss=s.test, Ve=0, 
                      Vg=var(test.pop.2$ie), 
                      M3g=moment(test.pop.2$ie, order=3, central=TRUE),
                      M4g=moment(test.pop.2$ie, order=4, central=TRUE))
...
test.rep.1.means.st <- apply(test.rep.1.p.st, MARGIN=2, mean)
ggplot(data.frame(dz=test.rep.1.means.st - mean(test.pop.2$ie))) + 
    geom_histogram(aes(x=dz), alpha=0.7) + theme_bw() + 
    geom_vline(xintercept=e.dz.st, color="red") + 
    geom_vline(xintercept=e.dz, color="blue") +
